...
import { saveCareGuide, seedStarterCareLibrary, setCareGuidePublished } from '../care-actions';
  ...
  return (
    <>
      ...
      <div className="admin-form-grid">
        ...
        <label className="admin-label">
          ...
          <select
            className="admin-input"
            name="guideType"
            defaultValue={guide?.guideType || CareGuideType.PLANT}
          >
        ...
        <label className="admin-label">
          ...
          <input
            ...
            placeholder="Trailing plants, Pests, Watering"
          />
        </label>
        <label className="admin-label">
          Difficulty or label
          <input
            ...
            placeholder="Beginner friendly, Common issue"
          />
        </label>
        <label className="admin-label">
          URL slug
        ...
        <label className="admin-label">
          ...
          <input
            className="admin-input"
            name="botanical"
            defaultValue={guide?.botanical || ''}
            placeholder="Optional; usually for a plant profile"
          />
        </label>
        <label className="admin-label">
          Display order
      ...
      <div className="admin-card" style={{ marginTop: 18 }}>
        ...
        <p className="muted">
          Fill these out for a specific plant. They can be left blank for general, problem and
          seasonal guides.
        </p>
        <div className="admin-form-grid">
          <label className="admin-label">
          ...
          <label className="admin-label">
            ...
            <textarea
              ...
              name="temperature"
              rows={2}
              defaultValue={guide?.temperature || ''}
            />
          </label>
          <label className="admin-label full">
      ...
      <div className="admin-card" style={{ marginTop: 18 }}>
        ...
        <p className="muted">
          These sections turn a guide into a practical diagnostic resource. They also work well for
          the common-problems section of a plant profile.
        </p>
        <div className="admin-form-grid">
          <label className="admin-label full">
      ...
      <div className="admin-form-grid" style={{ marginTop: 18 }}>
        ...
        <label className="admin-label full">
          Quick checklist — one item per line
          <textarea
            className="admin-input"
            name="checklist"
            rows={7}
            defaultValue={guide?.checklist || ''}
            placeholder={'Check soil moisture\nInspect leaf undersides\nReview recent changes'}
          />
        </label>
      </div>
      ...
      <div className="admin-actions">
        <label className="admin-checkbox">
          <input name="featured" type="checkbox" defaultChecked={guide?.featured ?? false} />{' '}
          Feature this guide
        </label>
        <label className="admin-checkbox">
          <input name="published" type="checkbox" defaultChecked={guide?.published ?? true} />{' '}
...
export default async function CareLibraryManager({
  ...
  const focusId = params.item;
  const banner =
    params.error === 'required'
      ? {
          tone: 'error' as const,
          text: 'A guide needs a title, an introduction and the main guidance.'
        }
      : params.seeded
      ...
      <div className="adminmain">
        <div className="toolbar">
          <div>
            ...
            <p className="muted">
              Manage plant profiles, general care lessons, common problems and seasonal checklists.
            </p>
          </div>
        ...
        {banner && (
          <div
            ...
            style={
              banner.tone === 'ok'
                ? { borderColor: 'var(--success)', background: '#f2faf4' }
                : undefined
            }
        ...
        <div className="statgrid">
          <div className="stat">
            ...
            <strong>{guides.filter((guide) => guide.published).length}</strong>
          </div>
          <div className="stat">
          ...
          <div className="stat">
            <span>Care basics</span>
            <strong>{counts.GENERAL}</strong>
          </div>
          <div className="stat">
            <span>Problem guides</span>
          ...
          <div className="stat">
            <span>Featured</span>
            <strong>{guides.filter((guide) => guide.published && guide.featured).length}</strong>
          </div>
        </div>
          ...
          <div className="care-admin-seed-card">
            <div>
              ...
              <p className="muted" style={{ margin: 0 }}>
                This safely creates or refreshes {starterCareGuides.length} professionally written
                guides covering common houseplants, core care skills, pests, root problems, leaf
                symptoms and seasonal care. It does not delete your additional guides.
              </p>
            </div>
        ...
        <section className="admin-section" id="guides">
          <div className="toolbar">
            <div>
              ...
              <p className="muted">
                Open a guide to edit any part of it. Unpublishing removes it from customers without
                deleting it.
              </p>
            </div>
          </div>
          ...
          <div className="admin-list">
            {guides.map((guide) => (
              <details
                key={guide.id}
                id={`guide-${guide.id}`}
                open={focusSlug === guide.slug || focusId === guide.id}
              >
                <summary>
                  <span>
                    {guide.plantName} • {careGuideTypeLabel(guide.guideType)}
                  </span>
                  <span className={`status-badge ${guide.published ? 'PAID' : 'CANCELLED'}`}>
