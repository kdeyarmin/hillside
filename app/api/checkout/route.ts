...
export async function POST(req:Request){
 try{
  ...
  const rawItems=typeof body==='object'&&body!==null&&'items' in body&&Array.isArray((body as {items?:unknown}).items)?(body as {items:unknown[]}).items:[];
  const requested:RequestedItem[]=rawItems.flatMap((x:unknown)=>{if(typeof x!=='object'||x===null)return[];const item=x as {id?:unknown;quantity?:unknown};const id=String(item.id||'').trim();if(!id)return[];return[{id,q:Math.max(1,Math.min(20,Number(item.quantity)||1))}]});
  const products=await db.product.findMany({where:{active:true,slug:{in:requested.map((i:RequestedItem)=>i.id)}}});
  const items=requested.flatMap((i:RequestedItem)=>{const p=products.find(v=>v.slug===i.id);if(!p||p.inventory<=0)return[];return[{p,q:Math.min(i.q,p.inventory)}]});
